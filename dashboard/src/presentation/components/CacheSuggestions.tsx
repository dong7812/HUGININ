    ...
    <div className="flex gap-3 py-3 border-b border-neutral-100 last:border-0">
      ...
      <div className="min-w-0">
        <div className="flex items-baseline gap-2">
          <span className="text-xs font-semibold text-neutral-900">{s.domain}</span>
          <span className="text-[10px] text-neutral-400">{s.count}회 반복</span>
        </div>
        <p className="text-[11px] text-blue-600 font-medium mt-0.5">→ {s.action}</p>
        {s.example && s.suggestionType === "domain" && (
          <p className="text-[10px] text-neutral-400 mt-1 truncate" title={s.example}>
            예: {s.example}
          </p>
        )}
        {s.example && s.suggestionType === "decision_type" && (
          <p className="text-[10px] text-neutral-400 mt-1">{s.example}</p>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-white border border-neutral-100 rounded-2xl shadow-sm p-4">
      <div className="flex items-center justify-between mb-3">
        <div>
          <h3 className="text-sm font-semibold text-neutral-900">캐시 전략 제안</h3>
          {data && (
      ...
      {isLoading ? (
        <div className="h-32 flex items-center justify-center">
          <span className="text-xs text-neutral-400">분석 중...</span>
        </div>
      ) : !data || data.suggestions.length === 0 ? (
        <div className="h-32 flex flex-col items-center justify-center gap-2">
          <p className="text-xs text-neutral-400">아직 분석할 데이터가 부족합니다</p>
          <p className="text-[10px] text-neutral-300">세션이 쌓이면 자동으로 패턴을 감지합니다</p>
        </div>
      ) : (
        <div>
          {paginated.map((s, i) => (
      ...
      {suggestions.length > 0 && (
        <div className="mt-3 flex items-center justify-between">
          <p className="text-[10px] text-neutral-400">
            <code className="text-neutral-500 bg-neutral-50 px-1 rounded">CLAUDE.md</code>에 추가하면 자동 참조됩니다
          </p>
          {totalPages > 1 && (
            <div className="flex items-center gap-0.5">
              <button
                ...
                disabled={page === 0}
                className="w-6 h-6 flex items-center justify-center rounded-lg text-neutral-400 hover:text-neutral-700 hover:bg-neutral-100 disabled:opacity-30 transition-colors"
              >
                <ChevronLeft size={12} />
              </button>
              <span className="text-[10px] font-mono text-neutral-400 tabular-nums w-8 text-center">
                {page + 1} / {totalPages}
              </span>
              <button
